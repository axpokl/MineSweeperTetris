...
{
    GetTextExtentPoint32A(_dc, s, (int)strlen(s), &_strz);
}
...
void DrawTextXY(pbitmap b, const short int* s, long x, long y, unsigned long w, unsigned long h, unsigned long cfg, unsigned long cbg, unsigned long format)
{
    ...
    lpRect.right = x + w;
    lpRect.bottom = y + h;
    DrawTextW(b->DC, (LPCWSTR)s, (int)wcslen((LPCWSTR)s), &lpRect, format);
    _fx = x + w;
    _fy = y;
}
void DrawTextXY(pbitmap b, const short int* s, long x, long y, unsigned long w, unsigned long h, unsigned long cfg, unsigned long cbg)
{
    ...
    lpRect.right = x + w;
    lpRect.bottom = y + h;
    DrawTextW(b->DC, (LPCWSTR)s, (int)wcslen((LPCWSTR)s), &lpRect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
    _fx = x + w;
    _fy = y;
}
void DrawTextXY(pbitmap b, const char* s, long x, long y, unsigned long w, unsigned long h, unsigned long cfg, unsigned long cbg)
{
    ...
    lpRect.right = x + w;
    lpRect.bottom = y + h;
    DrawTextA(b->DC, s, (int)strlen(s), &lpRect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
    _fx = x + w;
    _fy = y;
}
...
{
    InitTextXY(&b, &s, &cfg, &cbg);
    TextOutA(b->DC, x, y, s, (int)strlen(s));
